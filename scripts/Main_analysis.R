...
if (inherits(df_normalized, "try-error")) {
...
if (inherits(control_data, "try-error")) {
  print("Statistical testing error")
} else {
...
plot <- ggplot(merged_data, aes(x = Variability_Index, y = AnnexinV_avg_diff)) +
  geom_point() +
  theme_minimal() +
